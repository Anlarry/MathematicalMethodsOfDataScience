...
PCA（Principal components analysis）是一种统计分析、简化数据集的方法。它利用正交变换来
...
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate}
使用PCA降维之后我们成功的将原本的高维向量$V$降维到$d$维（$d=100$），在这之后，我们加入了星级评级$R$来
...
将数据进行整理得到月销售数据，为了得到平稳序列，对月销售数据进行2阶差分\autoref{fig:arima_diff}，通过ADF检验
